...
class CricketDataHandler {
    public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            Player player = new Player(data[0], Role.fromString(data[1]), Integer.parseInt(data[2]), Integer.parseInt(data[3]), data[4]);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
        ...
        List<Player> teamPlayers = new TeamFilterStrategy().filter(players, teamName);
        ...
        int totalRuns = teamPlayers.stream().mapToInt(Player::getRunsScored).sum();
        // Step 4: Compute and return the average runs scored. [1 mark]
        return totalRuns / teamPlayers.size();
    }
}
...
public class CricketAnalyticsSelfTest {
